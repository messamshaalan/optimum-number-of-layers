...
_WARMUP_5M    = 80    # 5-minute bars needed before first signal
_COOLDOWN_MIN = 30    # minutes between new entries (global)
...
class Backtester:
    def __init__(self,
        ...
        # High-res management bars: 10-second Heston ticks when available
    ...
    def _run_day(self, day: date) -> None:
        ...
        # Management bars: tick resolution if available, else 1 M
        if self._tick_mode:
        ...
        bar_1m_iter   = list(bars_1m.iterrows())   # kept for EOD fallback
        ...
        # EOD close: use last 1M bar (or last mgmt bar if tick mode)
        ...
        # Pass trades so PatternLog can update SessionTracker
...
def _best_signal(signals: List[Signal],
    ...
    if buy_score > sell_score:
        best = max((s for s in signals if s.direction == "BUY"),
                   key=lambda x: x.confidence)
        best.confidence = buy_score / total
    else:
        best = max((s for s in signals if s.direction == "SELL"),
                   key=lambda x: x.confidence)
