...
class DailyReview:
    ...
    def _observe(result: DailyResult,
        ...
        if result.strategy_results:
            ...
            if best_s.total > 0:
                obs.append(f"Best strategy: {best_s.name} (PnL ${best_s.total_pnl:+.2f}, {best_s.wins}W/{best_s.losses}L)")
            if worst_s.total > 0 and worst_s.name != best_s.name:
                obs.append(f"Worst strategy: {worst_s.name} (PnL ${worst_s.total_pnl:+.2f}, {worst_s.wins}W/{worst_s.losses}L)")
        ...
        if result.session_pnl:
            ...
            if worst_session != best_session:
                obs.append(f"Worst session: {worst_session} (${result.session_pnl[worst_session]:+.2f}) — reduce exposure")

        sl_streak = _count_sl_streak(trades)
        if sl_streak >= 3:
            obs.append(f"Warning: {sl_streak} consecutive SL hits — market may be ranging or strategy trading against trend")

        high_conf_losers = [t for t in trades if t.exit_reason == "SL" and t.confluence_score >= 0.80]
        if high_conf_losers:
            obs.append(f"{len(high_conf_losers)} high-confidence signal(s) hit SL — review confluence scoring; potential false breakouts")

        eod_profitable = [t for t in trades if t.exit_reason == "EOD" and (t.pnl_usd or 0) > 0]
        if eod_profitable:
            obs.append(f"{len(eod_profitable)} EOD-closed trade(s) were profitable — consider extending hold time for winning trades")
